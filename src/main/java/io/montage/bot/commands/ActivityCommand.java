...
public class ActivityCommand extends Command {
	...
	public void executeAndHandle (GuildMessageReceivedEvent event, List<String> params) {
		...
		try {
			...
			EmbedUtil.sendAndDeleteOnGuilds(event.getChannel(), new MessageBuilder("Bot Status Updated.")
					.build(), 15, TimeUnit.SECONDS);
			event.getMessage().delete();
		}
		catch (Exception e) {
			...
			EmbedUtil.sendAndDeleteOnGuilds(event
					.getChannel(), new MessageBuilder("Some issues happened while updating Status.  \n\n " + b.toString())
							.build(), 15, TimeUnit.SECONDS);
			event.getMessage().delete();
		}
	...
	public String getDescription () {
		return "Set bots status as you like. But be polite!";
	}
